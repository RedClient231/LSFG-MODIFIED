...
// Returns a human-readable explanation of WHY the most recent
// probe_shaders_on_device call returned kProbeDriverRejected (or empty if no
// rejection has been recorded). Includes the GPU name, driver-advertised
// shaderFloat16 / vulkanMemoryModel / shaderInt8 booleans, and the count of
// rejected shader modules. Used by the UI to replace the generic
// "GPU may not be compatible" toast with something actionable like
// "Mali-G57 driver lacks vulkanMemoryModel; try a r38+ firmware update".
//
// Lifetime: lives in a process-wide std::string. Safe to call from any
// thread that runs after probe_shaders_on_device. Not thread-safe to call
// concurrently with another probe_shaders_on_device.
