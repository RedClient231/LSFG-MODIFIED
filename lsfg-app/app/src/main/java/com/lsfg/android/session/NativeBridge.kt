...
object NativeBridge {
    ...
    /**
     * Returns a human-readable explanation of why the most recent [probeShaders] call
     * returned -12 (kProbeDriverRejected). Empty string if no rejection has been recorded
     * yet (e.g., probe hasn't run, or it succeeded). Includes the GPU name, the
     * driver-advertised shaderFloat16 / shaderInt8 / vulkanMemoryModel booleans, and the
     * count of rejected shader modules — enough for the UI to swap the generic
     * "Your GPU may not be compatible" toast for "Mali-G57 driver lacks vulkanMemoryModel".
     *
     * Process-wide state. Calling [probeShaders] from multiple threads at once is not
     * supported (and never has been — it tears a VkInstance up and down).
     */
    @JvmStatic
    external fun getProbeRejectReason(): String
